...
class WOOT {
    ...
    public function __construct() {
        ...
        if (is_admin()) {
            //$this->rate_alert = new WOOT_RATE_ALERT($this->show_notes);//replaced by ask_favour
        }
    }
    ...
    public function admin_enqueue_scripts() {
        ...
        if (isset($_GET['page']) AND $_GET['page'] == 'woot') {
            ...
            wp_enqueue_style('woot-fontello', WOOT_ASSETS_LINK . 'css/fontello.css', [], WOOT_VERSION);
            wp_enqueue_style('woot-options-fw', WOOT_ASSETS_LINK . 'css/admin/options-fw.css', [], WOOT_VERSION);
    ...
    public function draw_table_data($args, $table_html_id, $as_script = 'woot-table-json-data') {
        ...
        if ($this->show_notes) {
            ...
            if (isset($args['action'])) {
                if (in_array($args['action'], [
                            'woot_admin_table',
                            'woot_settings_table',
                            'woot_vocabulary_table',
                            'woot_columns_table',
                            'woot_predefinition_table',
                            'woot_columns_fields_options_table',
                            'woot_cart',
                            'woot_favourites',
                                //'woot_get_tables_meta',
                                //'woot_get_tables_options'
                        ])) {
                    $cut = false;
                }
            }
            ...
            if ($cut) {
                $columns = array_slice($columns, 0, $this->max_col);
            }
        }
        ...
        $js_script_data = [
            ...
            'request_data' => [
                ...
                'predefinition' => json_encode($predefinition),
                ...
                'shortcode_args_set' => json_encode($args),
    ...
    public function get_table_data($return = false, $requested_data = []) {
        ...
        if (empty($requested_data) && !empty($_REQUEST)) {
            ...
            foreach ($_REQUEST as $key => $value) {
                switch ($key) {
                    ...
                    case 'predefinition':
                        $predefinition = WOOT_HELPER::sanitize_array(json_decode((stripslashes($value)), true));
                        break;

                    case 'shortcode_args_set':
                        $shortcode_args_set = WOOT_HELPER::sanitize_array(json_decode((stripslashes($value)), true));
                        break;
                }
            }
        }
        ...
        if (isset($requested_data['shortcode_args_set'])) {
            $shortcode_args_set = WOOT_HELPER::sanitize_array(json_decode((stripslashes($requested_data['shortcode_args_set'])), true));
        }
                            ...
                            switch ($key) {
                                ...
                                case 'favourites_only':
                                    if (intval($value) > 0) {
                                        add_action('woot_wp_query_args', function ($args, $table_id) {
                                            $ids = [-1];
                                            global $WOOT_Favourites;

                                            if ($WOOT_Favourites) {
                                                $ids = $WOOT_Favourites->get_user_data();
                                            }
    ...
    public function get_smth() {
        ...
        switch ($what) {
            ...
            case 'shortcodes_set':
                $content = WOOT_HELPER::delete_not_allowed_shortcodes(esc_html($_REQUEST['shortcodes_set']));
        ...
        die('<div class="woot-content-in-popup"><p>' . $res . '</div>');
    }
    ...
    public function ask_favour() {
        if (get_option('woot_rate_alert', 0)) {
        ...
        add_action('admin_notices', function () use ($slug) {
            ...
            <div class="notice notice-info" id="pn_<?php echo $slug ?>_ask_favour" style="position: relative;">
                ...
                <div id="pn_<?php echo $slug ?>_review_yes" style="display: none;">
                    <p><?php printf(__('That\'s awesome! Could you please do us a BIG favor and give it a 5-star rating on %s to help us spread the word and boost our motivation?', 'profit-products-tables-for-woocommerce'), $on) ?></p>
                    <p style="font-weight: bold;">~ PluginUs.NET developers team</p>
                    <p>
                        <a href="<?php echo $link ?>" style="display: inline-block; margin-right: 10px;" onclick="pn_<?php echo $slug ?>_dismiss_review(2)" target="_blank"><?php esc_html_e('Okay, you deserve it', 'profit-products-tables-for-woocommerce'); ?></a>
                        <a href="javascript: pn_<?php echo $slug ?>_dismiss_review(1); void(0);" style="display: inline-block; margin-right: 10px;"><?php esc_html_e('Nope, maybe later', 'profit-products-tables-for-woocommerce'); ?></a>
                        <a href="javascript: pn_<?php echo $slug ?>_dismiss_review(2); void(0);"><?php esc_html_e('I already did', 'profit-products-tables-for-woocommerce'); ?></a>
                    </p>
                </div>

                <div id="pn_<?php echo $slug ?>_review_no" style="display: none;">
                    <p><?php _e('We are sorry to hear you aren\'t enjoying WOOT. We would love a chance to improve it. Could you take a minute and let us know what we can do better?', 'profit-products-tables-for-woocommerce'); ?></p>
                    <p>
                        <a href="https://pluginus.net/contact-us/" onclick="pn_<?php echo $slug ?>_dismiss_review(2)" target="_blank"><?php esc_html_e('Give Feedback', 'profit-products-tables-for-woocommerce'); ?></a>&nbsp;
                        |&nbsp;<a href="javascript: pn_<?php echo $slug ?>_dismiss_review(2); void(0);"><?php esc_html_e('No thanks', 'profit-products-tables-for-woocommerce'); ?></a>
                    </p>
                </div>
