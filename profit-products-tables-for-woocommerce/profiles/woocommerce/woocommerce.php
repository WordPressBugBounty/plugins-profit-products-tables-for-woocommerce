                        ...
                        if (is_a($product, 'WC_Product_Variation')) {
                            ...
                            if ($as_text) {
                                ...
                                return $labels . WOOT_HELPER::draw_html_item('a', array(
                                            ...
                                            'target' => apply_filters('woot_link_target_type', '_blank')
                                                ), $product->get_title());
                            }
                        }
                    }
...
//						return strftime('%d %B %Y', strtotime($post->post_date));
            ...
            'sku' => [
                ...
                'filter' => true,
                'filter_view' => 'textinput',
                'filter_options' => ['placeholder', 'search-logic', 'width'],
                'action' => function ($post_id) {
                    return $this->get_field_data('sku', $post_id);
                },
                'get_filter_query_args' => function ($args, $value)use ($table_id) {

                    $compare = 'LIKE';
                    if ($table_id > 0) {
                        $compare_tmp = woot()->filter->fields_options->get($table_id, "sku-search-logic");
                        if ($compare_tmp AND -1 != $compare_tmp) {
                            $compare = $compare_tmp;
                        }
                    }
